...
if __name__ == '__main__':
    ...
    res = min(results.values())
    imgs = [k for k, v in results.iteritems() if v == res][0]

    print 'Least similar images: %s %s (%.2f %%)' % (imgs[0], imgs[1], res)
    
    with Image.open(imgs[0]) as im:
        result_width, result_height = im.size
    ...
    for index, file in enumerate(imgs):
      ...
      img = Image.open(path)
      img.thumbnail((result_width*2, result_height), Image.ANTIALIAS)
      x = index * result_width
      y = 0
      w, h = img.size
      #print('pos {0},{1} size {2},{3}'.format(x, y, w, h))
      result.paste(img, (x, y, x + w, y + h))

    result.save(os.path.expanduser('image2.png'))